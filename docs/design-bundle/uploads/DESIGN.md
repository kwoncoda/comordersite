...
--color-card-muted:   #48402C   카드 내부 보조 텍스트 (2026-05-14 명도 조정: #5C5040 → #48402C, 대비 5.0:1 통과)
--color-card-divider: #9C8C68   카드 내부 구분선

  ★ 카드 내 형광 옐로 텍스트 금지 (2026-05-14 결정, §4.3 #8-B 대비 1.31:1):
    카드 위에 형광 옐로는 *배경*으로만 사용 (검정 텍스트 on 옐로 배경 패턴).
    형광 옐로 텍스트가 필요하면 *페이지 베이스(군복 톤)* 위에만.
...
--color-info:     #3A6B7E   슬레이트 블루 ℹ️ 안내
  ...
  ★ 사용 규칙 (2026-05-14 §4.3 #5 대비 2.37:1):
    --color-danger는 *군복 베이스 위 텍스트로는 사용 X* (대비 부족).
    대신: ① 빨간 배경 + 흰/밝은 텍스트 패턴
          ② 텍스트는 24px+ Bold만 (3:1 통과 — 큰 텍스트 기준)
          ③ 카드 영역 내에선 사용 가능 (#5 미달은 군복 베이스 한정)
...
### 4.3 대비 검증 (WCAG AA) — *2026-05-14 군복 톤 9 조합 실측 완료*

> **2026-05-14 결정 d 수용:** Claude가 9 조합 sRGB 상대 명도(relative luminance) 계산 완료. 본 표는 WCAG 2.1 공식 (`L = 0.2126*R + 0.7152*G + 0.0722*B` 선형 변환 후, contrast = (L1+0.05)/(L2+0.05))로 계산한 *실측값*. 정확도 ±0.3:1.

**페이지 베이스 (군복 톤) — 5 조합 실측:**
...
| 4-A | `--color-accent (#F4D200)` 위 검정 텍스트 (CTA 패턴) | **14.08:1** | ✅ |
| 4-B | `--color-accent` *텍스트* on `--color-bg` (옐로 글자 on 군복) | **8.10:1** | ✅ |
| 5 | `--color-danger (#C73E1D)` 텍스트 on `--color-bg` | **2.37:1** | ❌ **미달 — 조정 필요** |
...
| 7 | `--color-card-muted (#5C5040)` on `--color-card-bg` | **4.02:1** | ⚠️ **미달 — 명도 조정 필요** |
| 8-A | `--color-accent (#F4D200)` 위 검정 텍스트 (CTA on 흙색 카드) | **14.08:1** | ✅ |
| 8-B | `--color-accent` *텍스트* on `--color-card-bg` (옐로 글자 on 흙색) | **1.31:1** | ❌ **금지 — 보이지 X** |
| 9 | `--stamp-red (#B5301A)` on `--color-card-bg` | **3.16:1** | ⚠️ 큰 텍스트만 (18px+ Bold) — 도장 형식 OK |

**조정 권장 (3건):**

1. **#5 `--color-danger` 텍스트 미달** → `--color-bg` 위에 *텍스트로는 X*. 대신 **빨간 배경 + 흰 텍스트 패턴** 강제. 텍스트는 *24px+ Bold만 허용* (3:1 통과). §4.1 갱신.
2. **#7 `--color-card-muted` 미달** → `#5C5040` → **`#48402C`로 명도 조정** (대비 ~5.0:1 통과). §4.1 갱신.
3. **#8-B 카드 내 형광 옐로 텍스트 금지** → 카드 위에는 *옐로 배경 + 검정 텍스트* 패턴만. §4.1·§11 슬롭 #26 추가.

**기존 라이트 default 기준 (2026-05-10 검증, 운영진 PC 보조 모드 검토용):**
- `--color-ink-light (#2A2C20)` on `--color-bg-light (#F5EFE0)`: ~14:1 ✅
- 본 라이트 모드는 *운영진 사이드 PC 데이터 밀도용 검토* (1차 운영 후, §4.1 참조)

**검증 도구:**
- 본 표는 계산식 기반 실측. 추가 검증 시 WebAIM Contrast Checker (https://webaim.org/resources/contrastchecker/) 또는 Chrome DevTools "Inspect → Color picker → contrast ratio".
...
### 9.6 sessionStorage 모션 재생 규약 — *2026-05-14 결정 h 수용*

**문제:** 일부 단발 모션(도그태그 떨어짐)이 *세션당 1회*만 의미 있음. 반복 재생 시 *절정 UX 소진*. 다른 단발 모션(미니맵 펄스·CTA 깜박)은 *상태 신호*라 매번 재생이 자연.

**결정 (2026-05-14 결정 h):**

| 모션 | sessionStorage 정책 | 키 형식 |
|---|---|---|
| **도그태그 떨어짐** (`DogTagFrame`) | **세션당 1회** | `sessionStorage['dogtag-shown-{orderId}'] = 'true'` |
| 본인 테이블 펄스 (미니맵 `BoothMinimapModal`) | **매번 재생** | (sessionStorage X) — 모달 진입 시마다 위치 안내 신호 |
| StartBusinessCTA 빨간 깜박 (CLOSED + 16:30 이후 5분+) | **매번 재생** | (sessionStorage X) — 긴급 알림 상태 신호 |
| 마스코트 idle 흔들 (조리 중) | **매번 재생** | (sessionStorage X) — 살아있음 신호 |
| 도장 찍힘 (`StampBadge`) | **매번 재생** | (sessionStorage X) — 상태 등장 신호 |
...
const key = `dogtag-shown-${orderId}`;
if (!sessionStorage.getItem(key)) {
  // 첫 진입: 떨어지는 모션 재생
  $el.classList.add('dogtag-animate');
  sessionStorage.setItem(key, 'true');
} else {
  // 재방문: 정적 표시
  $el.classList.add('dogtag-static');
}

// 미니맵 펄스 / CTA 깜박 — sessionStorage 체크 없음, 항상 재생
```

**철학 (2026-05-14 결정 h 근거):**
- 도그태그 = *절정 의식* (1회) — 새로고침 시 매번 떨어지면 의식 가치 ↓
- 펄스·깜박 = *상태 안내 신호* — 매번 재생이 *현재 상태*를 알리는 의미
- 모션 종류별로 *의미*가 다르므로 *통일 규약 X*
...
## 10.5 메뉴 일러스트 PUBG 매핑 8종 — *2026-05-14 결정 b 확정*

> **사용자 결정 (2026-05-14):** 메뉴 *이름은 본명 유지* (G10), *시각·일러스트만* PUBG 배틀그라운드 인게임 회복 아이템과 매칭. 사용자가 PUBG 인게임 이미지 직접 찾아 제공 예정.

```
┌─────────────────────────────────────────────────────────────────┐
│ 메뉴 본명 (G10)         │ PUBG 인게임 아이템 매핑                  │
├─────────────────────────────────────────────────────────────────┤
│ 후라이드                │ 🩹 붕대 (Bandage)                       │
...
│ 칠리스                  │ 💉 아드레날린 주사기 (Adrenaline)       │
│ 콜라                    │ 💊 진통제 (Painkiller)                  │
│ 사이다                  │ 🥤 에너지 드링크 (Energy Drink)         │
└─────────────────────────────────────────────────────────────────┘
```

**중요 원칙 (사용자 메모리·G10 정합):**
- 메뉴 *이름은 본명 유지* — "후라이드"·"양념" 등 그대로 표기 (G10)
- *이미지·일러스트만* PUBG 회복 아이템 시각으로 매칭 — 사용자가 PUBG 인게임 캡처 또는 일러스트 직접 찾아 제공
- 주문·결제·이체 매칭은 *본명*으로 진행 (Pattern B 가격 계산, 이체 신고 이름)
- 사용자가 한눈에 *어느 PUBG 아이템이 어느 메뉴*인지 인식하는 *시각 차별화*만 의도

**카드 시각 톤:**
- 메뉴 카드 (`MenuCard`, COMPONENT §4.1)의 일러스트 영역 (4:3 비율)
- 흙색 카드 배경 + PUBG 아이템 시각
- 메뉴명 = 본명 (16px Bold Pretendard)
- "줍기" CTA = 한글 카피 (G11)

**자산 정책 (D-3 마감, 2026-05-17):**
- 사용자가 PUBG 인게임 캡처 또는 일러스트 8종 제공 (PNG/JPG)
- 미수령 시 fallback (ADR-006 정합): 분류 이모지 (🍗 치킨 + 🍟 사이드 + 🥤 음료) + 컬러 배경
- 런타임 교체 가능 (이미지 경로 1줄 변경, FEATURE_LIST §1 카탈로그 갱신)

**저작권 가드 (ADR-013 정합):**
- PUBG 공식 *로고/캐릭터*는 사용 X — 회복 *아이템 일러스트*는 *일반적 의료 도구* 시각이라 자체 제작 또는 무료 라이선스 일러스트 OK
- 사용자 제공 이미지가 *PUBG 인게임 캡처*면 *학교 축제 비영리·교내 한정*이라 저작권 위험 낮음 (단 D-day 2일 후 인쇄·외부 공개 X)
...
| **26** | **카드 영역 내 형광 옐로 텍스트** | *2026-05-14 §4.3 실측*: 흙색 카드 위 옐로 글자 대비 1.31:1 (보이지 X). 카드에선 *옐로 배경 + 검정 텍스트 패턴*만. 옐로 글자는 *페이지 베이스(군복 톤)* 위에만 |
...
| **2026-05-14** | **9 결정 사용자 확정 + Claude 실측 적용** — d 대비 실측 9 조합·5/7 미달 조정 / e 미니맵 상단+하단 닫기 / f "줍기" 버튼만 / g WINNER 폰트 현행 유지 / h 도그태그만 세션당 1회 / i 정산 200ms fade / a PNG 약도 / b 메뉴 PUBG 매핑 8종 / c 마스코트 이모지 fallback | 사용자 결정 |
| **2026-05-14** | **§4.1 색상 조정** — `--color-card-muted` #5C5040→#48402C (대비 4.02→5.0), `--color-danger` 사용 규칙 추가 (군복 베이스 텍스트 X, 배경+큰 텍스트만), 카드 내 형광 옐로 텍스트 금지 규칙 | §4.3 실측 #5·#7·#8-B 미달 |
| **2026-05-14** | **§9.6 sessionStorage 모션 재생 규약 신규** — 도그태그만 세션당 1회, 펄스·깜박·idle·도장은 매번 | 결정 h |
| **2026-05-14** | **§11 AI 슬롭 #26 카드 내 옐로 텍스트** 추가 (총 26개) | §4.3 #8-B 실측 |
